...
## 1. Product Meaning
SteadLog means knowledge gained through action. In homesteading, meaningful knowledge comes from repeated real-world work: planting, feeding, treating, harvesting, observing, and adapting.

SteadLog is a homestead memory system that captures those actions over time so users can answer:
- What did I do?
- When did I do it?
- What worked?
- What failed?
...
SteadLog is intentionally narrow and human-centered.
...
- All product and UX decisions must follow [SteadLog Design Philosophy](./STEADLOG_DESIGN_PHILOSOPHY.md) and pass the Barn Test.

Timeline doctrine:
- The emotional center of SteadLog is the timeline; all core modules should strengthen the living history model defined in [SteadLog Timeline Architecture](./STEADLOG_TIMELINE_ARCHITECTURE.md).
...
| Current capability | Evidence in codebase | SteadLog alignment | Action |
|---|---|---|---|
...
| Gamification (XP, achievements) | `src/game/*`, `src/pages/Achievements.tsx`, `src/components/game/*` | Partially aligned | Retain reinforcement backbone; rename/reframe as SteadLog Score + milestones |
...
To align with SteadLog philosophy before adding new logic:
...
SteadLog direction is succeeding when:
